...
(function () {
  ...
  PluginManager.registerCommand(PLUGIN_NAME, "ChangeParallax", function (args) {
    ...
    const spriteset = getSpriteset();
    // 対象の遠景を取得
    const targetParallaxes = spriteset._parallaxPlus.filter(
      (p) => id == null || p.id == id
    );
    // 変更を反映
    for (const parallax of targetParallaxes) {
      ...
      parallax.parallaxTile = changeValue(
        parallax.parallaxTile,
        params.parallaxTile
      );

      // 特殊な調整をする項目
      parallax.originalWidth = changeValue(
        parallax.originalWidth,
        params.width
      );
      parallax.originalHeight = changeValue(
        parallax.originalHeight,
        params.height
      );
  ...
  PluginManager.registerCommand(PLUGIN_NAME, "RemoveParallax", function (args) {
    ...
    // 遠景を削除
    // ※ＩＤが未指定の場合は全てが対象
    const deleteParallaxes = spriteset._parallaxPlus.filter(
      (p) => id == null || p.id == id
    );
  ...
  ParallaxPlus.prototype.setMapPosition = function (diffX, diffY) {
    ...
    if (this.mapX != null) {
      // 表示座標 = 配置座標 - 現在の画面座標
      this.x =
        (this.mapX - $gameMap.displayX()) * $gameMap.tileWidth() + this.shiftX;
      // 横ループマップの場合
    ...
    if (this.mapY != null) {
      // 表示座標 = 配置座標 - 現在の画面座標
      this.y =
        (this.mapY - $gameMap.displayY()) * $gameMap.tileHeight() + this.shiftY;
      // 縦ループマップの場合
