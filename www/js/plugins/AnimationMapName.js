...
(function () {
  ...
  Game_Interpreter.prototype.pluginCommand = function (command, args) {
    ...
    if (
      command === "AnimationMapName" ||
      command === "アニメーションマップネーム"
    ) {
      var code = args.shift();
  ...
  DataManager.amnMapParams = function (map) {
    ...
    for (var i = 0, max = texts.length; i < max; i++) {
      if (
        /^<(?:AnimationMapNameSetting|アニメーションマップネーム設定)>/.exec(
          texts[i]
        )
      ) {
        for (var j = i; j < max; j++) {
            ...
            if (/(.+?):(.+)?/.exec(texts[j])) {
              ...
              switch (code) {
                case "size":
                  ...
                  break;
                case "count":
                  params.count = Number(value);
                  break;
                case "ox":
                  params.ox = Number(value);
                  break;
                case "oy":
                  params.oy = Number(value);
                  break;
                case "mx":
                  params.mx = Number(value);
                  break;
                case "my":
                  params.my = Number(value);
                  break;
                case "pattern":
    ...
    args.forEach(
      function (a) {
        if (/(.+?):(.+)?/.exec(a)) {
          ...
          switch (code) {
            case "size":
              amnParams.size = Number(value);
              break;
            case "count":
              amnParams.count = Number(value);
              break;
            case "ox":
              amnParams.ox = Number(value);
              break;
            case "oy":
              amnParams.oy = Number(value);
              break;
            case "mx":
              amnParams.mx = Number(value);
              break;
            case "my":
              amnParams.my = Number(value);
              break;
            case "pattern":
              amnParams.pattern = value;
              break;
            case "color":
              amnParams.color = value;
              break;
            case "picture":
              amnParams.picture = value;
              break;
            case "text":
              amnParams.text = value;
              break;
          }
        }
      }.bind(this)
  ...
  Game_System.prototype.amnParameters = function () {
    ...
    var amnParams = {
      size: this._amnParameters.size,
      count: this._amnParameters.count,
      ox: this._amnParameters.ox,
      oy: this._amnParameters.oy,
      mx: this._amnParameters.mx,
      my: this._amnParameters.my,
      pattern: this._amnParameters.pattern,
      color: this._amnParameters.color,
      picture: this._amnParameters.picture,
    };
    for (var key in params) {
      amnParams[key] = params[key];
    }
    return amnParams;
  };
  ...
  Game_System.prototype.initAmnParameters = function () {
    ...
    if (patternVariableId) {
      ...
      if (pattern === "Normal" || pattern === 0)
        $gameVariables._data[patternVariableId] = 0;
      if (pattern === "Stretch" || pattern === -1)
        $gameVariables._data[patternVariableId] = -1;
      if (pattern === "GrowUp" || pattern === -2)
        $gameVariables._data[patternVariableId] = -2;
    }
  };
  ...
  Game_Interpreter.prototype.refreshMapName = function (params) {
    ...
    var array = CommonPopupManager.window()
  ...
  Game_Interpreter.prototype.showAmn = function (args) {
    ...
    var amnParams = DataManager.makeAmnParams(
      $gameSystem.amnParameters(),
      args
    );
  ...
  Game_Player.prototype.regionAmnArray = function (ri) {
    ...
    for (var i = 1, max = 255; i < max; i++) {
      ...
      if ($dataMap.meta["リージョンネーム" + i])
        $gameTemp._regionAmnArray[i] = true;
    }
